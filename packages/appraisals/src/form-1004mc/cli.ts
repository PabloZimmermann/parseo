import { parseForm1004MC } from "./parser.js";
...
async function main() {
  ...
  if (args.length === 0) {
    console.error("Usage: npx tsx appraisals/form-1004mc/cli.ts <pdf-path> [--output <json-path>]");
    process.exit(1);
  }
  ...
  const report = await parseForm1004MC(buffer);
