import { parsePNCStatement } from "./parser.js";
...
async function main() {
  ...
  if (args.length === 0) {
    console.error("Usage: npx tsx bank-statements/pnc/cli.ts <pdf-path> [--output <json-path>]");
    process.exit(1);
  }
  ...
  const report = await parsePNCStatement(buffer);
